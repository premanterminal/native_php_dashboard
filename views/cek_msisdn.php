...
if(isset($_POST['btn_cek']))
{
	...
	elseif($program=="data_bnum")
	{
		if(getDataBnum($bulan, $msisdn))
		{
			$success = "Cek Mkios Data Base on B# telah ditambahkan.";
			echo $success;
			$daftar_data_bnum = getDataBnum($bulan_x,$msisdn_x);
			$vas = json_decode($daftar_data_bnum, true);
		}
	...
	elseif($program=="bulk_bnum")
	{
		if(getBulkBnum($bulan, $msisdn))
		{
			$success = "Cek Mkios Bulk Base on B# telah ditambahkan.";
			echo $success;
			$daftar_bulk_bnum = getBulkBnum($bulan_x,$msisdn_x);
			$vas = json_decode($daftar_bulk_bnum, true);
		}
	...
	elseif($program=="legacy_bnum")
	{
		if(getLegacyBnum($bulan, $msisdn))
		{
			$success = "Cek Mkios Legacy Base on B# telah ditambahkan.";
			echo $success;
			$daftar_legacy_bnum = getLegacyBnum($bulan_x,$msisdn_x);
			$vas = json_decode($daftar_legacy_bnum, true);
		}
...
switch($_GET['pilih'])
{
	...
	$daftar_legacy_bnum = getLegacyBnum($bulan_x,$msisdn_x);
	$vas = json_decode($daftar_legacy_bnum, true);
    //$daftar_ = getvas($bulan_x,$msisdn_x);
	//$vas = json_decode($daftar_, true);
	//echo $flash;
	...
	<div class="col-lg-12 col-md-12">
		...
		<section class="pb-4">
			<div class="card">
					...
					<div class="card">
						<h3 class="card-header teal white-text">CEK MKIOS VAS BASE ON B#</h3>
						<div class="card-body">
						<br>
								<p>
								...
								<div class="col-md-12">
									...
									<div class="md-form">
										...
										<select name="program" class="mdb-select" required>
											...
											<option value="bulk_bnum">BULK</option>
											<option value="data_bnum">DATA</option>
											<option value="legacy_bnum">LEGACY</option>
