...
switch($_GET['pilih'])
{
    ...
    try
	{
		...
		select a.Branch, a.Cluster, count(distinct a.Outlet) as jlh_outlet,
		sum(a.Target) as target, sum(a.Voice) as voice, sum(a.Bulk) as bulk, 
		sum(a.Total) as total, (sum(a.Total)/sum(a.Target))*100 as ach_rev,
		b.jlh_oa, (b.jlh_oa/count( distinct a.Outlet))*100 as ach_oa
		from SIKAT a
		left join
		(
		  select Cluster,count(distinct Outlet) as jlh_oa from SIKAT where total >= '50000' 
		  group by Cluster
		) b
		...
		");
		$stmt->execute();
		$sikat = $stmt->fetchAll(PDO::FETCH_ASSOC);
	}
...
<main>
    <section class="pb-4">
        <div class="card">
        	<h3 class="card-header red white-text">SIKAT      <?Php echo"$newdate";?> SEPTEMBER 2018 </h3>
            <div class="card-body">
				
				<table class="table table-striped table-responsive" id="dataTables">
				
				<center>				
				<p style="font-size:16px;font-family:verdana;text-align:center;font-size:16px"><b><a href="http://10.33.97.177/tiramisu/autoemail_mcd/sikatnew_export.php">Download for Detail SIKAT</a></b><form action="http://10.33.97.177/tiramisu/autoemail_mcd/sikatnew_export.php">
            	</center>
